...
class Settings(BaseSettings):
    ...
    def validate_webhook_url(cls, v):
        """Webhook URL validatsiyasi - HTTPS shart"""
        if v is None:
            return v
        v = v.strip()
        if not v:
            return None
        # HTTPS tekshirish (Telegram talabi)
        if not v.startswith("https://"):
            raise ValueError("Webhook URL must use HTTPS (Telegram requirement)")
        # URL formati tekshirish
        import re
        url_pattern = re.compile(
            r'^https://[a-zA-Z0-9][-a-zA-Z0-9]*(\.[a-zA-Z0-9][-a-zA-Z0-9]*)+(/.*)?$'
        )
        if not url_pattern.match(v):
            raise ValueError(f"Invalid webhook URL format: {v}")
        return v
