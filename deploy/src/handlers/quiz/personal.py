...
async def quiz_start(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(
            "📚 <b>Quiz boshlash</b>\n\n"
            "📊 Darajani tanlang:",
            reply_markup=await level_keyboard(levels)
        )
...
async def select_language(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "📚 <b>Quiz boshlash</b>\n\n"
        "📊 Darajani tanlang:",
        reply_markup=await level_keyboard(levels, language_id)
    )
    await callback.answer()
...
"""
    ...
    try:
        await bot.send_message(
            chat_id=chat_id,
            text=text,
            reply_markup=await quiz_result_keyboard(show_review=wrong > 0)
        )
