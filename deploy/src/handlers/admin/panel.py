...
def is_admin(user_id: int) -> bool:
    """Check if user is admin or super admin (settings only)"""
    return user_id in settings.ADMIN_IDS or is_super_admin(user_id)


async def is_admin_async(user_id: int) -> bool:
    """Check if user is admin - database va settings dan tekshirish"""
    # Settings dan tekshirish
    if user_id in settings.ADMIN_IDS or is_super_admin(user_id):
        return True
    ...
    async with get_session() as session:
        ...
        is_db_admin = result.scalar_one_or_none()
        return bool(is_db_admin)
...
def admin_menu_keyboard(is_super: bool = False) -> InlineKeyboardMarkup:
    ...
    if is_super:
        ...
        builder.row(
            InlineKeyboardButton(text="🔘 Tugmalar", callback_data="admin:buttons")
        )
...
# ============================================================
# BUTTON MANAGEMENT - Dinamik tugma nomlari
# ============================================================

@router.callback_query(F.data == "admin:buttons")
async def admin_buttons_menu(callback: CallbackQuery):
    """Button management menu"""
    if not is_super_admin(callback.from_user.id):
        await callback.answer("❌ Faqat Super Admin!", show_alert=True)
        return

    from src.services.button_service import BUTTON_CATEGORIES

    text = """
🔘 <b>Tugmalar boshqaruvi</b>

Bot tugmalarining nomlarini o'zgartirish mumkin.
...
"""
    ...
    for cat_id, cat_info in BUTTON_CATEGORIES.items():
        builder.row(InlineKeyboardButton(
            text=f"📂 {cat_info['name']}",
            callback_data=f"admin:btn_cat:{cat_id}"
        ))

    builder.row(InlineKeyboardButton(text="🔄 Hammasini tiklash", callback_data="admin:btn_reset_all"))
    builder.row(InlineKeyboardButton(text="◀️ Orqaga", callback_data="admin:panel"))
...
@router.callback_query(F.data.startswith("admin:btn_cat:"))
async def admin_buttons_category(callback: CallbackQuery):
    """Show buttons in a category"""
    if not is_super_admin(callback.from_user.id):
        await callback.answer("❌ Faqat Super Admin!", show_alert=True)
        return
    ...
    from src.services.button_service import ButtonTextService, BUTTON_CATEGORIES, DEFAULT_BUTTONS

    if cat_id not in BUTTON_CATEGORIES:
        await callback.answer("❌ Kategoriya topilmadi!", show_alert=True)
        return

    cat_info = BUTTON_CATEGORIES[cat_id]
    buttons = await ButtonTextService.get_by_category(cat_id)

    text = f"📂 <b>{cat_info['name']}</b>\n\n"
    text += "Tugmani tanlang:\n\n"

    builder = InlineKeyboardBuilder()

    for key in cat_info["keys"]:
        current_value = buttons.get(key, DEFAULT_BUTTONS.get(key, key))
        default_value = DEFAULT_BUTTONS.get(key, "")

        # Qisqartirish
        display = current_value[:20] + "..." if len(current_value) > 20 else current_value

        # O'zgartirilganmi?
        modified = " ✏️" if current_value != default_value else ""

        builder.row(InlineKeyboardButton(
            text=f"{display}{modified}",
            callback_data=f"admin:btn_edit:{key}"
        ))

    builder.row(InlineKeyboardButton(text="◀️ Orqaga", callback_data="admin:buttons"))
...
async def admin_button_edit(callback: CallbackQuery, state: FSMContext):
    ...
    key = callback.data.replace("admin:btn_edit:", "")

    from src.services.button_service import ButtonTextService, DEFAULT_BUTTONS

    current = await ButtonTextService.get(key)
    default = DEFAULT_BUTTONS.get(key, "")

    await state.update_data(editing_button_key=key)
    await state.set_state(AdminStates.waiting_button_text)

    text = f"""
✏️ <b>Tugmani tahrirlash</b>
...
"""
    ...
    builder = InlineKeyboardBuilder()
    if current != default:
        builder.row(InlineKeyboardButton(text="🔄 Defaultga tiklash", callback_data=f"admin:btn_reset:{key}"))
    builder.row(InlineKeyboardButton(text="❌ Bekor qilish", callback_data="admin:buttons"))
...
async def admin_button_save(message: Message, state: FSMContext):
    """Save new button text"""
    data = await state.get_data()
    key = data.get("editing_button_key")
    ...
    if not new_text:
        await message.answer("❌ Bo'sh matn kiritish mumkin emas!")
        return

    from src.services.button_service import ButtonTextService

    success = await ButtonTextService.set(key, new_text)
    await state.clear()
...
async def admin_button_reset(callback: CallbackQuery, state: FSMContext):
    """Reset button to default"""
    if not is_super_admin(callback.from_user.id):
        await callback.answer("❌ Faqat Super Admin!", show_alert=True)
        return

    key = callback.data.replace("admin:btn_reset:", "")

    from src.services.button_service import ButtonTextService, DEFAULT_BUTTONS

    await ButtonTextService.reset_to_default(key)
    default = DEFAULT_BUTTONS.get(key, "")
...
async def admin_buttons_reset_all(callback: CallbackQuery):
    ...
    text = """
⚠️ <b>Barcha tugmalarni tiklash</b>

Barcha tugma matnlari default qiymatlariga tiklanadi.
...
"""
    ...
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="✅ Ha, tiklash", callback_data="admin:btn_reset_all_confirm"),
        InlineKeyboardButton(text="❌ Yo'q", callback_data="admin:buttons")
    )
...
@router.callback_query(F.data == "admin:btn_reset_all_confirm")
async def admin_buttons_reset_all_confirm(callback: CallbackQuery):
    """Confirm reset all buttons"""
    if not is_super_admin(callback.from_user.id):
        await callback.answer("❌ Faqat Super Admin!", show_alert=True)
        return

    from src.services.button_service import ButtonTextService, BUTTON_CATEGORIES
