...
async def start_command(message: Message, db_user: User, **kwargs):
    ...
    if not_subscribed:
        ...
        await message.answer(
            ...
            f"{channels_text}",
            reply_markup=await channel_subscription_keyboard(not_subscribed)
        )
        return
...
async def quick_start(callback: CallbackQuery, db_user: User, state: FSMContext):
    ...
    # Yangilangan foydalanuvchi ma'lumotlarini olish (onboarding dan keyin yangilanmagan bo'lishi mumkin)
    async with get_session() as session:
        user_repo = UserRepository(session)
        fresh_user = await user_repo.get_by_user_id(callback.from_user.id)
        if fresh_user:
            db_user = fresh_user
    ...
    try:
        ...
        # Agar day_id bo'lmasa, level_id orqali barcha kunlardan savollar olinadi
        if db_user.current_day_id:
            ...
            # day_id yo'q - level bo'yicha barcha kunlardan
            await state.update_data(
                day_id=None,
                selected_level_id=db_user.current_level_id,
                all_days=True,
...
"""
        ...
        await callback.message.edit_text(
            text,
            reply_markup=await question_count_keyboard(back_callback="menu:main")
        )
        await callback.answer()
...
"""
    ...
    await message.answer(help_text, reply_markup=await main_menu_keyboard())
...
"""
    ...
    await message.answer(text, reply_markup=await premium_menu_keyboard(is_premium))
...
"""
    await callback.message.edit_text(
        welcome_text,
        reply_markup=await onboarding_keyboard()
    )
    await callback.answer()
...
"""
    await callback.message.edit_text(
        guide_text,
        reply_markup=await onboarding_guide_keyboard()
    )
    await callback.answer()
...
"""
                await callback.message.edit_text(
                    text,
                    reply_markup=await day_keyboard(days, level_id=a1_level["id"])
                )
...
"""
                await callback.message.edit_text(
                    text,
                    reply_markup=await day_keyboard(days, level_id=selected_level["id"])
                )
