...
async def check_tournament_end(bot: Bot = None):
    """Turnir tugashini tekshirish va yangi boshlash"""
    from src.services import tournament_service, finish_expired_tournaments
    from src.core.logging import get_logger
    logger = get_logger(__name__)

    try:
        # Tugagan turnirni yakunlash
        result = await finish_expired_tournaments()
        if result:
            logger.info(f"Tournament finished: {result}")

            # G'oliblarni xabardor qilish (agar bot berilgan bo'lsa)
            if bot and result.get("winners"):
                for winner in result.get("winners", [])[:3]:
                    try:
                        await bot.send_message(
                            winner["user_id"],
                            f"🏆 <b>Tabriklaymiz!</b>\n\n"
                            f"Siz haftalik turnirda #{winner['rank']} o'rinni egalaladingiz!\n"
                            f"🎁 Sovg'a: {winner.get('prize', 'N/A')}",
                            parse_mode="HTML"
                        )
                    except Exception:
...
def setup_scheduler(bot: Bot):
    ...
    scheduler.add_job(
        ...
        args=[bot],  # Bot parametrini uzatish
        id="check_tournament",
