...
def _cleanup_expired_memory():
    ...
    with _memory_lock:
        ...
        # Bir marta iterate qilib, expired keylarni topish va o'chirish
        keys_to_delete = []
        for key, (value, expire_at) in _memory_store.items():
        ...
        current_size = len(_memory_store)
        if current_size > MAX_MEMORY_ITEMS:
            items_to_remove = current_size - MAX_MEMORY_ITEMS
            # Sort qilishdan oldin list() nusxasini olish
            sorted_keys = sorted(
                list(_memory_store.keys()),
                key=lambda k: _memory_store.get(k, (None, float('inf')))[1] or float('inf')
            )
    ...
    if cleaned_count > 0:
        logger.debug(f"Memory cleanup: {cleaned_count} items removed")
