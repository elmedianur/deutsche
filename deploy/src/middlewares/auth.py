...
class AuthMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        async with get_session() as session:
            ...
            # Barcha kerakli ma'lumotlarni oldindan yuklash - lazy load oldini olish
            try:
                # Streak va uning barcha atributlarini yuklash
                if db_user.streak:
                    _ = db_user.streak.current_streak
                    _ = db_user.streak.longest_streak
                    _ = db_user.streak.last_activity_date

                # Subscription va uning barcha atributlarini yuklash
                if db_user.subscription:
                    _ = db_user.subscription.plan
                    _ = db_user.subscription.expires_at
                    _ = db_user.subscription.is_active
            except Exception as e:
                logger.debug(f"Relationship preload warning: {e}")

            # Ma'lumotlarni dictionary sifatida saqlash - lazy load muammosini to'liq hal qilish
            user_data = {
            ...
            # User data ni ham saqlash (lazy load muammosi uchun fallback)
